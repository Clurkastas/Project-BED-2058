...
temp <- stringr::str_detect(WDI_datasets[["name"]], "pollution")
...
WDI_list <- list(WDI_ch_mor, 
                ...
                WDI_sec_f,  WDI_pri_out,
                #WDI_road,   #thrown out due missings
                WDI_pois ,   WDI_murder,
                WDI_gini,   WDI_y_unemp, WDI_ad_unemp,  WDI_int_unemp,
                WDI_bas_unemp,
                #WDI_gdp,    WDI_gdpgr,  WDI_water_stress, #thrown out due missings
                WDI_renewable_water,     WDI_CO2,       WDI_air)
...
dat2 <- aggregate(dat, by=list(dat$country,dat$decade),
  FUN=mean, na.rm=TRUE)
dat2 <- dat2[,c(1:2,10:12,14:length(dat2))]
dat2 <- dat2[dat2$decade>=1990,]
...
m1 <- '
health =~ lifeExp + child_mor + surv_65f + surv_65m + cook
education =~ pri_ed + sec_ed + sec_f + pri_out
security =~ poison + murder
economy =~ y_unemp + ad_unemp + int_unemp + bas_unemp
environemt =~ renew_wat + CO2 + air
'
fit.m1 <- lavaan(m1, dat2, estimator = "MLR", missing = "fiml")
summary(fit.m1, fit = T, std =T)
modindices(fit.m1)
