...
knitr::opts_chunk$set(echo = F, include = F, warning=F, error=F)
...
thismap <- left_join(thismap, temp2, 
                     by = c("region" = "country")) 
ggplot(thismap, aes(long, lat, group=group, fill=meta)) +
  scale_fill_gradient(limits=range(thismap$meta), low="violetred2", high="chartreuse") + 
...
econ_data_norway <- econ_data[ which(econ_data$country=="Norway"),]
econ_data_singapore <- econ_data[ which(econ_data$country=="Singapore"),]
econ_data_ireland <- econ_data[ which(econ_data$country=="Ireland"),]


library(reshape2)
econ_data <- reshape2::melt(econ_data, id.var = 'year')
